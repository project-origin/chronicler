...
{
    public class ChroniclerRepositoryTests : IClassFixture<PostgresDatabaseFixture<Startup>>, IDisposable
    {
        ...
        {
            // Act
            await _repository.DeleteClaimIntent(Guid.NewGuid());
        }
        ...
        {
            // Arrange
            var registry = _fixture.Create<ReadBlock>();

            // Act
            await _repository.UpsertReadBlock(registry);

            // Assert
            _con.QuerySingle<ReadBlock>("SELECT * FROM read_blocks").Should().BeEquivalentTo(registry);
        }
        ...
        public async Task UpsertReadBlock_UpdatesBlock()
        {
            ...
            var updatedRegistry = new ReadBlock
            {
            ...
            // Assert
            _con.QuerySingle<ReadBlock>("SELECT * FROM read_blocks").Should().BeEquivalentTo(updatedRegistry);
        }
        ...
        {
            // Arrange
            var registry1 = _fixture.Create<ReadBlock>();
            var registry2 = _fixture.Create<ReadBlock>();
